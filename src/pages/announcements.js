...
class Announcements extends React.Component {
  ...
  componentDidMount(){
    var edges = this.props.data.allSanityAnnouncement.edges
    var posts = []
  ...
  getNext(){
    const query = '*[_type == "announcement"] | order(_createdAt desc) | [$nextIndex...$nextPlusFive]'
  ...
  render() {
    ...
    var ArchiveItems = this.props.data.archive.edges.map((itemData) =>
      <li className="ArchiveItem" key={itemData.node._id}>
        <Link to={'/announcement?' + itemData.node.slug.current}>{itemData.node.title}</Link>
      </li>
    )
    return (
      <div id="body">
...
export const query = graphql`{
  allSanityAnnouncement(sort: {fields: _createdAt, order: DESC}, limit: 5) {
    edges {
      node {
        _createdAt
        title
  ...
  archive: allSanityAnnouncement(sort: {fields: _createdAt, order: DESC}) {
    edges {
      node {
        _createdAt
        title
